...
static pthread_mutex_t __dtoa_locks[2];
...
rjs_dtoa_init (void)
{
    pthread_mutex_init(&__dtoa_locks[0], NULL);
    pthread_mutex_init(&__dtoa_locks[1], NULL);
}
...
{
    pthread_mutex_destroy(&__dtoa_locks[0]);
    pthread_mutex_destroy(&__dtoa_locks[1]);
}

#define ACQUIRE_DTOA_LOCK(n) pthread_mutex_lock(&__dtoa_locks[n])
#define FREE_DTOA_LOCK(n)    pthread_mutex_unlock(&__dtoa_locks[n])

#define dtoa_get_threadno()  1
...
#ifdef __cplusplus
extern "C" double rjs_strtod(const char *s00, char **se);
extern "C" char *rjs_dtoa(double d, int mode, int ndigits,
...
dtoa_r(double dd, int mode, int ndigits, int *decpt, int *sign, char **rve, char *buf, size_t blen)
{
 ...
 char *
rjs_dtoa(double dd, int mode, int ndigits, int *decpt, int *sign, char **rve)
{
